build {
  ...
  provisioner "ansible" {
    ...
    playbook_file    = "${var.os_anpb}/playbooks/001650-debian13/debian13_postinstall.yml"
  }
  provisioner "ansible" {
    user             = "${var.os_user}"
    extra_arguments  = ["-e","h=default","-e","ansible_ssh_pass=${var.os_pass}","-b"]
    playbook_file    = "${var.os_anpb}/playbooks/001650-debian13/debian13_postinstall_software.yml"
  }
  provisioner "ansible" {
    user             = "${var.os_user}"
    extra_arguments  = ["-e","h=default","-e","ansible_ssh_pass=${var.os_pass}","-b"]
