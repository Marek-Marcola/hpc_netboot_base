build {
  ...
  provisioner "shell" {
    ...
    inline = [
      "set -x",
      "V=$(curl ${var.os_dopt} ${var.os_durl}/sw/nvidia/cuda_toolkit/version.txt|grep ^cuda-repo-rhel9:|tail -1|awk '{print $2}')",
      "curl ${var.os_dopt} -o /tmp/f.rpm ${var.os_durl}/sw/nvidia/cuda_toolkit/cuda-repo-rhel9-$V.x86_64.rpm",
      "rpm -ih /tmp/f.rpm",
      "rm -fv /tmp/f.rpm",
